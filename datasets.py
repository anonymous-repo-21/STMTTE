...
class MyDifferDatasetWithEmbedding:
    def __init__(self, dataset, batch_size):
        ...
        self.dists = dists[:len(dists) - (len(dists) % batch_size)]
    ...
    def get_batch(self):
        ...
        dists = self.dists[self.current_ptr: self.current_ptr + self.batch_size]
        ...
        features = [
            np.stack([lats[i], lngs[i], timeIDs[i], weekIDs[i], dists[i]], axis=1) for i in range(len(lats))
        ]
